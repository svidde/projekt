...
class CMForum extends CObject implements IHasSQL,  IModule   {
	...
	public static function SQL($key=null) {
  	  ...
  	  $queries = array(
  	  	  ...
  	  	  'create table forum'        => "CREATE TABLE IF NOT EXISTS Forum (id INTEGER PRIMARY KEY, status INTEGER, message TEXT, author TEXT, created DATETIME default (datetime('now') ) );",
  	  	  'insert forum'              => 'INSERT INTO Forum (status,message,author) VALUES (?,?,?);',
    ...
    switch($action) {
      case 'install': 
        try {
          ...
          $this->db->executeQuery(self::SQL('insert forum'), array( 0, "Testar ett meddelande", "Pelle"));
          $this->db->executeQuery(self::SQL('insert forum'), array( 0, "Testar ännu ett meddelande", "Lisa"));
          $this->db->executeQuery(self::SQL('insert forum'), array( 1, "Nu ska detta bli en kommentar", "Kalle"));
          $this->db->executeQuery(self::SQL('insert forum'), array( 2, "Även detta ska bli en kommentar", "Tina"));
          $this->db->executeQuery(self::SQL('insert forum'), array( 0, "Ett superfint meddelande", "Stina"));
  ...
  public function addForum($message, $author) 
  {
	  ...
	  {
		  $this->db->executeQuery(self::SQL('insert forum'), array( 0, $message, $author));
  ...
  public function addComment($message, $author, $id) 
  {
	  ...
	  {
		  $this->db->executeQuery(self::SQL('insert forum'), array( $id, $message, $author));
		  $this->session->addMessage('success', 'Successfully inserted new message.');
