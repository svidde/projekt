...
function login_menu() {
	...
	if($dr->user['isAuthenticated']) {
		$items = "<a href='" . create_url('my/profile') . "'><img class='gravatar' src='" . get_gravatar(20) . "' alt=''> " . $dr->user['acronym'] . "</a> ";
		if($dr->user['hasRoleAdmin']) {
			$items .= "<a href='" . create_url('my/acp') . "'>acp</a> ";
		}
		$items .= "<a href='" . create_url('my/logout') . "'>logout</a> ";
	} else {
		$items = "<a href='" . create_url('my/login') . "'>login</a> ";
	}
